...
```{r}
# Central Valley Data
CEDEN.tissue.CV <- fread("Data/CV_Tissue_202012911342.txt") %>%
  ...
  select(any_of(CEDEN.selectList)) %>%
  mutate(Date = as_date(ymd(Date))) # Format date to conform to SFB Table
...
CEDEN.toxicity <- full_join(CEDEN.toxicity.CV, CEDEN.toxicity.SFB) %>%
  mutate(Date = as_date(Date))
CEDEN.toxicity <- CEDEN.toxicity[,c(8,3,1,2,15,11,9,10,4,14,3,16,17,12,13)] # reorder columns; easier/more consistent way to do this with dplyr::relocate?

tibble(1:length(names(CEDEN.toxicity)), names(CEDEN.toxicity))
```
