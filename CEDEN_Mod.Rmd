...
**10/20/2022 NOTE: Skyler edits to run new expanded time frame = NAME**
...
# Load Data
CEDEN.toxicity <-
  fread("Data/CEDEN_TOX_26JUN2022.txt",
                        quote = "") %>%
...
```{r}
# Load Data
CEDEN.WQ <- fread("Data/CEDEN_WQ_26JUN2022.txt", 
...
Earlier run resulted in failed expansion of timeframe, and fewer WQ samples total. How did this do?
...
```{r}
# Compare toxicity output
nrow(Tox.old)
nrow(CEDEN.toxicity.sf)

min(as.Date(Tox.old$Date))
min(as.Date(CEDEN.toxicity.sf$Date))

max(as.Date(Tox.old$Date)) 
max(as.Date(CEDEN.toxicity.sf$Date)) 
```
...
min(as.Date(WQ.old$Date)) 
min(as.Date(CEDEN.WQ.sf$Date)) 

max(as.Date(WQ.old$Date)) 
max(as.Date(CEDEN.WQ.sf$Date))
```
