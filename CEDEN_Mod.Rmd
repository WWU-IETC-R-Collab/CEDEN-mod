...
st_crs(USFE.regions) # CRS should be GCS NAD83
...
# Load Data
CEDEN.toxicity <-
  fread("Data/CEDEN/original/CEDEN_TOX_26JUN2022.txt",
                        quote = "") %>%
  select(any_of(CEDEN.selectList))
```
...
write_csv(CEDEN.toxicity.sf, "Data/Output/CEDENMod_Toxicity_26JUN2022.csv") # Note: coerces empty data fields to NA
```
...
```{r}
# Load Data
CEDEN.WQ <- fread("Data/CEDEN/original/CEDEN_WQ_26JUN2022.txt",
                  quote = "") %>%
  select(any_of(CEDEN.selectList)) %>%
...
write_csv(CEDEN.WQ.sf, "Data/Output/CEDENMod_WQ_26JUN2022.csv") # Note: coerces empty data fields to NA
```
